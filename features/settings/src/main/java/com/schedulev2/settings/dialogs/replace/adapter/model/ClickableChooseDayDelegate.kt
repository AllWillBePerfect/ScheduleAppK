package com.schedulev2.settings.dialogs.replace.adapter.model
...
import com.schedulev2.settings.databinding.V2ItemChooseDayBigBinding
...
class ClickableChooseDayDelegate(private val onItemClickListener: (DayChooseItem.ClickableChooseDay) -> Unit) : AdapterItemDelegate<DayChooseItem>, OnClickListener {
    ...
    override fun onClick(p0: View?) {
        if (p0?.id == com.schedulev2.settings.R.id.v2_item_choose_day_big) {
        ...
        private fun changeBackgroundColor(
            ...
            if (item.isSelected)
                binding.root.background = ContextCompat.getDrawable(
                    context,
                    com.schedulev2.settings.R.drawable.part_choose_day_bg
                )
            binding.root
        }
    }
