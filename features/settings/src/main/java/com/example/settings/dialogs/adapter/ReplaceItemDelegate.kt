...
import com.example.settings.databinding.V2ItemReplaceBinding
import com.example.settings.dialogs.adapter.model.GroupItem
import com.example.settings.dialogs.replace.adapter.ChooseDayDelegate
...
class ReplaceItemDelegate(private val onItemClickListener: (GroupItem.Replace) -> Unit) : AdapterItemDelegate<GroupItem>, OnClickListener {
    ...
    override fun onClick(p0: View?) {
        if (p0?.id == com.example.settings.R.id.v2_item_replace) {
    ...
    inner class ReplaceItemViewHolder(private val binding: V2ItemReplaceBinding) :
        ...
        fun bind(item: GroupItem.Replace) {
            ...
            binding.vpkName.text = if (item.isSelected) " | ${item.vpkName} *" else " | ${item.vpkName}"
