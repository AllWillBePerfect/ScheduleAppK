package com.example.settings.dialogs.replace.adapter
...
import com.example.settings.databinding.V2ItemChooseDayBinding
...
class ChooseDayDelegate(
    ...
    override fun onClick(p0: View?) {
        if (p0?.id == com.example.settings.R.id.v2_item_choose_day)
        ...
        private fun changeBackgroundColor(
            ...
            if (item.isSelected)
                binding.root.background = ContextCompat.getDrawable(
                    context,
                    com.example.settings.R.drawable.part_choose_day_bg
                )
            binding.root
        }
    }
